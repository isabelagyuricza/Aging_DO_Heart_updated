########### Chromosome 3 hotspot - QTL analysis on the DO heart ################

# Analyzing the correlation between proteins on the hotspot on chromosme 3 
...
ggplot(lod.peaks, aes(x=qtl.pos, y=lod)) +
  geom_point() +
  xlim(145,149) #Region with the highest lod scores ~145Mb to 149Mb
#
...
# Saving data to compare with the proteins from the hotspot on chr 12 

saveRDS(lod.peaks, 
        file = "Downstream_Analysis/QTL_mapping/Results/proteins_hotspot_chr3.RDS")
...
test <- expr.data %>% 
  ...
  mutate(protein.id = gsub("\\..*","", protein.id)) %>% 
  left_join(lod.peaks) %>% 
...
pdf("Downstream_Analysis/QTL_mapping/Results/Correlation_hotspot_chr3.pdf",
    width = 10,
    height = 10)
...
#3_146965364 
#12760  Using this marker for the model.

# Getting the interval of the hotspot
bayes_int(scan1_output = scan.out, lodcolumn = 3, prob = 0.99,map = map, chr = 3)
# ci_lo      pos    ci_hi
# 1 146.3567 146.9654 148.3054
...
fit <- fit1(genoprobs$`3`[,,"3_146965364"], pheno$PC1, kinship = K[3],
...
fit_2 <- fit1(genoprobs$`3`[,,"3_146965364"], pheno$PC1, kinship = K[3],
              addcovar = ac, intcovar = ic, zerosum = FALSE)
...
fit_3 <- fit1(genoprobs$`3`[,,"3_146965364"], pheno$PC1, kinship = K[3],
              addcovar = ac, intcovar = ic, zerosum = FALSE)
...
pdf("Downstream_Analysis/QTL_mapping/Results/PC1_hotspot_chr3.pdf")
...
write.csv(save, "Downstream_Analysis/QTL_mapping/Results/Enrichment_hotspot_chr3.csv",
          row.names = FALSE)
...
# About 20% of the proteins that mapped on the QTL are enriched for some protein
# modification/regulation category.
...
pdf("Downstream_Analysis/QTL_mapping/Results/Enrichment_hotspot_chr3.pdf",
    width = 8, height = 6)
